...
import { Search, X, ChefHat, Clock } from 'lucide-react';
import { clsx } from 'clsx';
import { getCategories, getPopularItems, searchItems } from '../services/api';
import { Counter, AddButton, ItemBadge, Spinner, formatPrice } from '../components/ui';
import FoodDetailModal from '../components/FoodDetailModal';
import FloatingCartButton from '../components/FloatingCartButton';
import useCartStore from '../stores/cartStore';
import { useTelegram } from '../hooks/useTelegram';

// ─── Food Card (горизонтальная) ───
const FoodCard = ({ item, onOpen }) => {
    ...
    return (
        ...
        >
            {/* Image */}
            <div className="w-[88px] h-[88px] rounded-xl bg-slate-100 shrink-0 overflow-hidden relative">
                {item.image_url ? (
...
// ─── Popular Card (компактная, горизонтальный скролл) ───
const PopularCard = ({ item, onOpen }) => {
    const { addItem, removeItem, getQuantity } = useCartStore();
    const { haptic } = useTelegram();
    ...
    return (
        <div
            ...
            className={clsx(
                ...
                isDisabled && 'grayscale opacity-60' // Стили для деактивации, если товара нет
            )}
        >
            {/* Изображение */}
            <div className="w-full aspect-[4/3] bg-slate-50 relative overflow-hidden">
                ...
                {/* Бейдж статуса (например, "Нет в наличии") */}
                {isDisabled && (
                    <div className="absolute inset-0 bg-black/40 flex items-center justify-center">
                        <span className="text-white text-[10px] font-bold uppercase tracking-tighter">Закончилось</span>
                    </div>
                )}
            </div>
                ...
                {showSearch && (
                    <div className="px-4 pb-3 animate-fadeIn">
                        <div className="relative">
                            ...
                            <input
                                autoFocus
                                className="w-full h-11 pl-10 pr-10 bg-surface rounded-xl text-[15px] placeholder:text-slate-400 outline-none"
                    ...
                    <div ref={tabsRef} className="flex gap-2 px-4 pb-3 overflow-x-auto scrollbar-hide">
                        {categories.map(cat => (
                            <button
                                ...
                                className={clsx(
                                    'shrink-0 px-4 py-2 rounded-full text-[14px] font-medium transition-all duration-200',
                ...
                {displayItems !== null ? (
                    <div>
                        <p className="text-sm text-slate-400 mb-3">
                            {searchLoading ? 'Поиск...' : `Найдено: ${displayItems.length}`}
                        </p>
                    ...
                    <>
                        ...
                        {popular.length > 0 && (
                            <div className="mb-6">
                                ...
                                <div className="flex gap-3 overflow-x-auto scrollbar-hide pb-2">
                                    {popular.map(item => (
                                        <PopularCard key={item.id} item={item} onOpen={setSelectedItem} />
                                    ))}
                                </div>
                            </div>
                        )}
                        ...
                        {categories.map(cat => {
                            ...
                            return (
                                ...
                                >
                                    <h2 className="text-[18px] font-semibold text-slate-800 mb-3">
                                        {cat.icon} {cat.name_ru}
                                    </h2>
                                    {items.map(item => (
                                        <FoodCard key={item.id} item={item} onOpen={setSelectedItem} />
