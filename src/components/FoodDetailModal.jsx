...
import { Counter, AddButton, ItemBadge, Backdrop, formatPrice } from './ui';
    ...
    return (
        <Backdrop onClick={onClose}>
            <div className="fixed bottom-0 left-0 right-0 z-50 bg-white rounded-t-3xl animate-slideUp max-h-[90vh] overflow-y-auto safe-bottom">
                ...
                {/* Close button */}
                <button onClick={onClose} className="absolute top-4 right-4 z-10 w-8 h-8 bg-white/80 backdrop-blur rounded-full flex items-center justify-center shadow-sm">
                ...
                {item.image_url ? (
                    <div className="relative w-full h-60 bg-slate-100">
                        <img src={item.image_url} alt={item.name_ru} className="w-full h-full object-cover" />
                        <ItemBadge status={item.status} />
                    </div>
                ) : (
                    <div className="w-full h-40 bg-slate-100 flex items-center justify-center text-5xl">🍽</div>
                )}
